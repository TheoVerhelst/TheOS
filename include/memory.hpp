...
/// hold all things related to the implementation of the memory management.
...
namespace details
{
	...
	class MemoryBlock
	{
		public:
			...
			void removeFromList();
			void tryMergeBlock(MemoryBlock** blockArray, size_t index);
	};
	...
	/// The index indicate the size of the blocks in the list:
	/// a block at index 4 has a size of 2^4 bytes.
	MemoryBlock* allocatedBlocks[addressSize];
	...
	MemoryBlock memoryBlocksArray[maxBlocksNumber];
	...
	BitSet<maxBlocksNumber> usedMemoryBlocks;

	MemoryBlock* getBlock(size_t index);

	size_t getIndexFromSize(size_t size);

	Pair<MemoryBlock*, size_t> findBlock(MemoryBlock** blockArray, size_t index, void* address);
}
